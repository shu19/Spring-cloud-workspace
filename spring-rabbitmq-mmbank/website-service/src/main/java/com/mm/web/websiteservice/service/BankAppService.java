...
public class BankAppService {
	...
	@HystrixCommand(fallbackMethod = "fundtransfer")
	public void fundTransfer(int senderAccountNumber, int receiverAccountNumber, int amount, Model model) {

		restTemplate.postForEntity("http://localhost:8763/transactions/fundTransfer?sender=" + senderAccountNumber
	...
	public void fundtransfer(int senderAccountNumber, int receiverAccountNumber, int amount, Model model) {
		...
		model.addAttribute("message", "Server is down Please Try again after some time");
		System.out.println("Server is down Please Try again after some time");		
	}
	

	@HystrixCommand(fallbackMethod = "withdrawfail")
	public void withdraw(Transaction transaction, Model model) {
		restTemplate.postForEntity("http://localhost:8763/transactions/withdraw", transaction, null);
		model.addAttribute("message", "Success!");
		
	}
	public void withdrawfail(Transaction transaction, Model model) {
		System.out.println("Server is down Please Try again after some time");
		model.addAttribute("message", "Server is down Please Try again after some time");
	}
	...
	public void deposit(Transaction transaction, Model model) {
		restTemplate.postForEntity("http://TRANSACTION-SERVICE/transactions/deposit", transaction, null);
		model.addAttribute("message", "Success!");		
	}

	public void depositfail(Transaction transaction, Model model) {
		model.addAttribute("message", "Server is down Please Try again after some time");
		System.out.println("Server is down Please Try again after some time");
